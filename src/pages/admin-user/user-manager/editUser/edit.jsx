...
export default function EditUser({
  ...
  const [addressInfo, setAddressInfo] = useState(
    {
      ...
      add: selectedRecord?.address?.address,
      lat: selectedRecord?.address?.latitude,
      lng: selectedRecord?.address?.longitude,
    } || {},
  );
  ...
  const validationSchema = Yup.object({
    ...
    manabadiEmail: Yup.string(t('EMAIL_REQUIRED'))
      .email(t('EMAIL_INVALID'))
      .required(t('EMAIL_REQUIRED')),
  ...
  const formikData = selectedRecord;
  if (formikData?.contact_number) {
    formikData.contact_number = formikData.contact_number.charAt(0) === '+' ? formikData.contact_number : `+${formikData.contact_number}`;
  }
  const formik = useFormik({
    initialValues: {
      userId: formikData?.id,
      firstName: formikData?.first_name,
      lastName: formikData?.last_name,
      gender: formikData?.gender ? formikData?.gender : '',
      email: formikData?.personal_email ? formikData?.personal_email : '',
      manabadiEmail: formikData?.manabadi_email ? formikData.manabadi_email : '',
      contactNo: formikData?.contact_number,
      userStatus: formikData?.is_active || false,
      profileImage: formikData?.profileImage ? formikData?.profileImage : profilePic,
      changeLogs: '',
      roleType: formikData?.roles,
      // roleType: [],
      address: formikData?.address?.address,
    },
    validationSchema,
    onSubmit: (values) => {
      setLoading(true);
      ...
      if (values.email !== formik.initialValues.email) {
        payload.userDetails.personalEmail = values.email;
      }
      if (formik?.values?.profileImage !== formikData?.profileImage) {
        payload.userDetails.image = values.profileImage;
      }
      ...
      dispatch(
        updateUserInfo(userId, payload, refreshList, setLoading),
      ).then(() => {
        setDialogOpen(false);
      }).catch((err) => {
  ...
  useEffect(() => {
    const results = roleOptions?.filter((op) => selectedRecord?.roles?.some(
      (value) => op?.name === value.name,
    ));
              ...
              <Grid item xs={12} sm={12} md={7} lg={7}>
                <Grid container spacing={1}>
                  ...
                  <Grid item xs={12} md={12} lg={12} className={classes.alignGrid}>
                    <PerformantTextField
                      disabled
                      label={`${t('MANABADI_EMAIL')}`}
                      id="manabadiEmail"
                      name="manabadiEmail"
                      type="email"
                      value={formik?.values?.manabadiEmail}
                      onBlur={() => setPreviousFieldTouch('manabadiEmail')}
                      onChange={formik.handleChange}
                      labelId="manabadiEmail"
                  ...
                  <Grid item xs={12} md={6} lg={6} className={classes.alignGrid} alignSelf="center">
                    ...
                    <Stack direction="row" spacing={1} alignItems="center" justifyContent="center">
                      ...
                      <AntSwitch
                        ...
                        onChange={() => {
                          formik.setFieldValue('userStatus', !formik.values.userStatus);
                        }}
